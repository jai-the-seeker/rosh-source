...
The `phpinfo` page will show the location of the php configuration file `php.ini`. This path can also be mapped to the local vol in order to preserve any changes in the `php.ini` file. This can be done by inserting following line in the `docker-compose.yml` file under web/volume section.
```
