...
Copy the `Dockerfile` we created previously into the `php` forlder. `vol-db` and `vpl-php` will be used to map the database and php volumes. We will also create a user for MySql. Username `devuser` and password `devpass`.
